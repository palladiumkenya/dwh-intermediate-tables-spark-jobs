package org.kenyahmis.intermediatepbfw;
...
public class LoadPBFW {
    public static void main(String[] args) {
        ...
        try {
            ...
            if (conn != null) {
                String query = new LoadPBFW().loadQuery();
                Statement statement = conn.createStatement();
                statement.setQueryTimeout(Integer.parseInt(timeout));
                statement.execute(query);
                System.out.println("Loaded intermediate pbfw");
            } else {
    ...
    private String loadQuery() {
        String query;
        final String fileName = "LoadPBFW.sql";
        InputStream inputStream = LoadPBFW.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
