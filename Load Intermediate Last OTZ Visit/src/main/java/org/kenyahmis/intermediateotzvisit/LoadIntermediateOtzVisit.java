package org.kenyahmis.intermediateotzvisit;
...
public class LoadIntermediateOtzVisit {
    private static final Logger logger = LoggerFactory.getLogger(LoadIntermediateOtzVisit.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (conn != null) {
                String query = new LoadIntermediateOtzVisit().loadQuery();
                Statement statement = conn.createStatement();
                statement.setQueryTimeout(Integer.parseInt(timeout));
                statement.execute(query);
                logger.info("Loaded intermediate otz visit loads table");
            }else {
                throw new RuntimeException("Failed to connect to sql database");
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
            throw new RuntimeException("Failed to run intermediate otz visit query");
        } finally {
            try {
    ...
    private String loadQuery() {
        String query;
        final String fileName = "IntermediateLastOtzVisit.sql";
        InputStream inputStream = LoadIntermediateOtzVisit.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
